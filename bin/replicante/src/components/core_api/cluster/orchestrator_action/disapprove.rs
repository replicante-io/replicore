...
impl Disapprove {
    ...
    pub fn resource(&self) -> impl HttpServiceFactory {
        ...
        let tracer = TracingMiddleware::with_name(
            logger,
            tracer,
            "/cluster/{cluster_id}/orchestrator-action/{action_id}/disapprove",
        );
        web::resource("/orchestrator-action/{action_id}/disapprove")
...
async fn responder(
    ...
    let response = with_request_span(&mut request, |span| {
        super::load_transform_event_persist(
            ...
            |mut action| {
                ...
                // Approve the action.
                action.finish(OrchestratorActionState::Cancelled);
                Ok(action)
            },
        )
    })?;
    ...
    if response.is_none() {
        debug!(
            data.logger,
            "Disapproved (rejected) orchestrator action for scheduling";
            "cluster" => cluster_id,
            "action" => %action_id,
        );
    }
