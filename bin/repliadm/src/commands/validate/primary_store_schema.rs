...
pub fn command() -> App<'static, 'static> {
    SubCommand::with_name(COMMAND)
        .about("Validate the primary store schema")
}
...
pub fn run<'a>(args: &ArgMatches<'a>, interfaces: &Interfaces) -> Result<Outcomes> {
    ...
    debug!(logger, "Checking primary store indexes");
    ...
    debug!(logger, "Checking primary store for removed collections/tables or indexes");
