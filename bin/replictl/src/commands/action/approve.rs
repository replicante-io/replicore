...
pub const COMMAND: &str = "approve";

pub fn command<'a, 'b>() -> App<'a, 'b> {
    SubCommand::with_name(COMMAND).about("Approve an action pending approval")
}
...
pub fn run<'a>(cli: &ArgMatches<'a>, logger: &Logger) -> Result<()> {
    ...
    client.action_approve(cluster, action)?;

    println!("Action approved for scheduling");
    Ok(())
}
