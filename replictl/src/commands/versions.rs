...
fn replicante_versions<'a>(args: &ArgMatches<'a>, logger: &Logger) -> Result<()> {
    ...
    println!("Replicante (statically determined): {}", *REPLICANTE_VERSION);
    let version = Client::new(args)
        .and_then(|client| client.version())
        .map(|version| format!("{} [{}; {}]", version.version, version.commit, version.taint));
    println!(
...
fn coordinator_version(config: &Config, logger: &Logger) -> Result<()> {
    let version = CoordinatorAdmin::new(config.coordinator.clone(), logger.clone())
        ...
        .and_then(|admin| admin.version().with_context(|_| ErrorKind::FetchVersion("coordinator")))
        .map_err(Error::from);
    println!("Coordinator: {}", value_or_error(logger, "coordinator", version));
    Ok(())
}
