...
pub fn command() -> App<'static, 'static> {
    SubCommand::with_name(COMMAND)
        ...
        .subcommand(
            SubCommand::with_name(COMMAND_LS).about("List currently held non-blocking locks"),
        )
}
...
fn force_release<'a>(args: &ArgMatches<'a>, interfaces: &Interfaces) -> Result<()> {
    ...
    let confirm = interfaces.prompt().confirm_danger(&format!(
        "Are you sure you want to force-release non-blocking lock '{}'?",
        name,
    ))?;
    if !confirm {
